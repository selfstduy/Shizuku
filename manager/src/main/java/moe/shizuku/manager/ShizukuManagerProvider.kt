...
import moe.shizuku.manager.adb.HeadlessAdbCommandHandler
...
class ShizukuManagerProvider : ShizukuProvider() {
    ...
    private val headlessAdbCommandHandler by lazy(LazyThreadSafetyMode.NONE) {
        HeadlessAdbCommandHandler(requireNotNull(context))
    }
    ...
    override fun call(method: String, arg: String?, extras: Bundle?): Bundle? {
        if (headlessAdbCommandHandler.canHandle(method)) {
            return headlessAdbCommandHandler.handle(method, arg, extras)
        }
